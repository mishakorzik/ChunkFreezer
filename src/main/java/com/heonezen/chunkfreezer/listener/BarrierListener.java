...
public final class BarrierListener implements Listener {
    ...
    public void onLivingMove(EntityMoveEvent e) {
        ...
        if (chunkX(from) == tcx && chunkZ(from) == tcz) return;
    ...
    public void onPlayerMoveWithMount(PlayerMoveEvent e) {
        ...
        if (chunkX(from) == tcx && chunkZ(from) == tcz) return;
        if (!frozen.isFrozen(w, tcx, tcz)) return;

        e.setCancelled(true);
        e.setTo(from);

        final Location back = from.clone();
        final Vector oldVel = mount.getVelocity();

        mount.teleportAsync(back);
        mount.getScheduler().execute(plugin, () -> {
            if (!mount.isValid() || mount.isDead()) return;
            Vector push = oldVel.multiply(-settings.bounceMultiplier);
            if (push.lengthSquared() > 1.0E-6) mount.setVelocity(push);
            else mount.setVelocity(new Vector(0, 0, 0));
...
private void startWatching(Entity entity) {
    ...
    private void tickWatch(Item item, ScheduledTask task) {
        ...
        if (!frozen.isFrozen(w, chunkX(now), chunkZ(now))) {
            st.lastSafe = now.clone();
            st.lastVelocity = item.getVelocity();
            return;
        }
